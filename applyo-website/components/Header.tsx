  ...
  return (
    <header className="bg-gray-50">
      <nav aria-label="Global" className="mx-auto flex max-w-7xl items-center justify-between p-6 lg:px-8">
        <div className="flex lg:flex-1">
          <a className="-m-1.5 p-1.5">
            <span className="sr-only">Applyo</span>
            <Image
                src={logo}
                alt={`Applyo logo`}
                priority={true}
        ...
        <PopoverGroup className="hidden lg:flex lg:gap-x-12">
                ...
                {products.map((item) => (
                  ...
                  >
                    <div className="flex size-11 flex-none items-center justify-center rounded-lg bg-gray-700/50 group-hover:bg-gray-700">
                      <item.icon aria-hidden="true" className="size-6 text-gray-400 group-hover:text-black" />
                    </div>
              ...
              <div className="grid grid-cols-2 divide-x divide-white/10 bg-gray-700/50">
                {callsToAction.map((item) => (
                  <a
                    key={item.name}
                    href={item.href}
                    className="flex items-center justify-center gap-x-2.5 p-3 text-sm/6 font-semibold text-black hover:bg-gray-700/50"
                  >
          ...
          <p className="text-xl font-semibold text-black">
            No payments involved, free browser extension!
          </p>
        </PopoverGroup>
        <div className="hidden lg:flex lg:flex-1 lg:justify-end">
          <a onClick={() => login()} href="#" className="text-sm/6 font-semibold text-black">
      ...
      <Dialog open={mobileMenuOpen} onClose={setMobileMenuOpen} className="lg:hidden">
        ...
        <DialogPanel className="fixed inset-y-0 right-0 z-50 w-full overflow-y-auto bg-gray-50 p-6 sm:max-w-sm sm:ring-1 sm:ring-gray-100/10">
